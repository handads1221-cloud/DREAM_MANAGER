...
export type Plan = { id:string; schedule_date:string; schedule_time:string|null; title:string; details:string; created_at:string; updated_at:string; creator_name:string; kind:'manual'|'birthday'|'holiday'; editable:boolean };
...
export function PlansCalendar({ plans, today, canEdit }: { plans:Plan[]; today:string; canEdit:boolean }) {
  ...
  return <div className="plans-workspace"><div className="plans-toolbar"><div><button type="button" onClick={()=>move(-1)} aria-label="이전 달">‹</button><h2>{year}년 {month+1}월</h2><button type="button" onClick={()=>move(1)} aria-label="다음 달">›</button></div>{canEdit&&<button type="button" className="plan-add-button" onClick={()=>{setSelectedDate(today);setSelected(null);setForm('new');}}>＋ 일정등록</button>}</div><div className="plan-legend"><span className="manual">공유 일정</span><span className="birthday">생일</span><span className="holiday">공휴일</span></div>
    {canEdit && form === 'new' && <section className="plan-editor"><h3>새로운 일정</h3><PlanForm defaultDate={selectedDate} onCancel={()=>setForm(null)}/></section>}
    <section className="plans-calendar"><div className="plans-weekdays">{weekdays.map((day,index)=><b key={day} className={index===0?'sunday':index===6?'saturday':''}>{day}</b>)}</div><div className="plans-days">{cells.map((day,index)=>day===null?<div className="plan-day blank" key={`blank-${index}`}/>:<div key={day} className={`plan-day ${iso(year,month,day)===today?'today':''}`}><button type="button" className="plan-day-number" onClick={()=>{if(canEdit){setSelectedDate(iso(year,month,day));setSelected(null);setForm('new');}}}>{day}</button><div>{(byDate[iso(year,month,day)]??[]).slice(0,3).map(plan=><button key={plan.id} type="button" className={`plan-chip ${plan.kind}`} onClick={()=>{setSelected(plan);setForm(null);}}><small>{plan.schedule_time?.slice(0,5)}</small>{plan.title}</button>)}{(byDate[iso(year,month,day)]??[]).length>3&&<span className="plan-more">+{byDate[iso(year,month,day)].length-3}개</span>}</div></div>)}</div></section>
    {selected && <section className={`plan-detail ${selected.kind}`}><div className="plan-detail-heading"><div><p>{Number(selected.schedule_date.slice(5,7))}월 {Number(selected.schedule_date.slice(8,10))}일 · {timeText(selected.schedule_time)}</p><h2>{selected.title}</h2></div><span>{selected.creator_name}</span></div><p className="plan-detail-body">{selected.details || '등록된 상세내용이 없습니다.'}</p>{canEdit&&selected.editable&&(form==='edit'?<PlanForm plan={selected} defaultDate={selected.schedule_date} onCancel={()=>setForm(null)}/>:<div className="plan-detail-actions"><button type="button" onClick={()=>setForm('edit')}>일정 수정</button><form action={deletePlan} onSubmit={(event)=>{if(!confirm('이 일정을 삭제하시겠습니까?')) event.preventDefault();}}><input type="hidden" name="id" value={selected.id}/><button type="submit" className="danger">일정 삭제</button></form></div>)}</section>}
  </div>;
}

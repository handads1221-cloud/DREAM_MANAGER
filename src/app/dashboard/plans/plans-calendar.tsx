...
export function PlansCalendar({ plans, today }: { plans:Plan[]; today:string }) {
  ...
  return <div className="plans-workspace"><div className="plans-toolbar"><div><button type="button" onClick={()=>move(-1)} aria-label="이전 달">‹</button><h2>{year}년 {month+1}월</h2><button type="button" onClick={()=>move(1)} aria-label="다음 달">›</button></div><button type="button" className="plan-add-button" onClick={()=>{setSelectedDate(today);setSelected(null);setForm('new');}}>＋ 일정등록</button></div><div className="plan-legend"><span className="manual">직접 등록</span><span className="birthday">생일</span><span className="holiday">공휴일</span></div>

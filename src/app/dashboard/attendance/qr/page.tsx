...
function QrStage({ date, qrData, popup, noServiceReason }: { date: string; qrData: string; popup: boolean; noServiceReason?: string | null }) {
  ...
  return <main className={`qr-stage${popup ? ' popup' : ''}`}>
    ...
    <section className="qr-stage-content">
      ...
      {noServiceReason ? <div className="qr-no-service"><strong>오늘은 교회학교 별도 예배가 없습니다</strong><span>{noServiceReason}</span></div> : <>{qrData && <div className="qr-stage-code"><Image src={qrData} alt="오늘의 출석 QR코드" width={1040} height={1040} unoptimized priority/></div>}<p>핸드폰앱으로 모두 출첵해봐요^^</p></>}
    </section>
  </main>;
}
...
export default async function AttendanceQrPage({ searchParams }: PageProps<'/dashboard/attendance/qr'>) {
  ...
  todayDate.setUTCDate(todayDate.getUTCDate() + (7 - isoDay));
  const serviceDate = todayDate.toISOString().slice(0, 10);
  const { data: excludedEvent } = await supabase.from('attendance_events').select('statistics_exclusion_reason').eq('service_date', serviceDate).eq('is_statistics_excluded', true).maybeSingle();
  const { data: qrRows } = excludedEvent ? { data: null } : await supabase.rpc('admin_get_attendance_qr', {});
  const qr = qrRows?.[0];
  ...
  const stage = <QrStage date={qr?.service_date ?? serviceDate} qrData={qrData} popup={popup} noServiceReason={excludedEvent?.statistics_exclusion_reason}/>;

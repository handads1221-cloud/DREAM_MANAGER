...
const menus: Record<AppRole, MenuItem[]> = {
  ...
  parent: [{ icon: 'home', label: '홈', href: '/dashboard' }, { icon: 'child', label: '우리아이', href: '/dashboard/children' }, { icon: 'notice', label: '공지게시판', href: '/dashboard/notices' }, { icon: 'inquiry', label: '문의하기', href: '/dashboard#inquiry' }],
...
export function DashboardShell({ profile, activeHref = '/dashboard', children }: { profile: { full_name: string; role: AppRole }; activeHref?: string; children: ReactNode }) {
  ...
  return <main className="site-shell operation-shell">
    <header className="topbar">
      <Link href="/dashboard" className="brand student-home-brand" aria-label="내 홈으로"><Image className="church-brand-mark" src="/shinheung-church-mark.png" alt="청주신흥교회" width={48} height={48} priority/><div><small>청주신흥교회</small><strong>드림 어린이부</strong><small>{roleLabel[profile.role]} 운영 홈</small></div><span className="mobile-brand-title"><small>청주신흥교회</small><strong>드림어린이부</strong></span></Link>

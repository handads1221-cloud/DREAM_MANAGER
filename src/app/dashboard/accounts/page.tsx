...
import { approveRegistration, rejectRegistration, restoreAccount, sendAccountPasswordReset, updateAccountRole, withdrawAccount } from './actions';
...
export default async function AccountsPage({ searchParams }: PageProps<'/dashboard/accounts'>) {
  ...
  const [{ data: requests }, { data: accounts }] = await Promise.all([
    ...
    supabase.from('profiles').select('id, full_name, role, phone, is_active, account_status, withdrawn_at, withdrawal_note').order('full_name'),
  ]);
  const requestRows = requests ?? [];
  const emailByUserId = new Map(requestRows.map((request) => [request.user_id, request.email]));
  ...
  return <DashboardShell profile={{ full_name: profile.full_name, role: 'admin' }} activeHref="/dashboard/accounts">
    ...
    <section className="account-management-section"><div className="section-title-row"><div><h2>활성 계정</h2><p>권한 변경과 비밀번호 초기화를 관리합니다.</p></div><b>{activeAccounts.length}개</b></div><div className="managed-account-list">{activeAccounts.map((account) => <article key={account.id} className="managed-account-card"><div className="managed-account-person"><span>{account.full_name.slice(0, 1)}</span><div><h3>{account.full_name}</h3><p>{emailByUserId.get(account.id) ?? '이메일 정보 없음'} · {account.phone || '연락처 미입력'}</p></div><b className={`account-role ${account.role}`}>{roleLabel[account.role] ?? account.role}</b></div><div className="managed-account-actions"><form action={updateAccountRole}><input type="hidden" name="user_id" value={account.id} /><select name="role" defaultValue={account.role} aria-label={`${account.full_name} 권한 변경`} disabled={account.id === currentUserId}><option value="parent">부모님</option><option value="student">학생</option><option value="teacher">선생님</option><option value="admin">관리자</option></select><button type="submit" disabled={account.id === currentUserId}>권한 변경</button></form><form action={sendAccountPasswordReset}><input type="hidden" name="user_id" value={account.id} /><button type="submit" className="secondary">비밀번호 초기화 메일</button></form>{account.id !== currentUserId && <details className="withdraw-account"><summary>탈퇴 처리</summary><form action={withdrawAccount}><input type="hidden" name="user_id" value={account.id} /><input name="withdrawal_note" maxLength={200} placeholder="탈퇴 사유 (선택)" /><p>로그인만 차단되며 기존 출석·보석·관계 데이터는 삭제되지 않습니다.</p><button type="submit">탈퇴 계정으로 전환</button></form></details>}</div></article>)}{activeAccounts.length === 0 && <div className="account-empty">활성 계정이 없습니다.</div>}</div></section>

    <section className="account-management-section withdrawn"><div className="section-title-row"><div><h2>탈퇴 계정</h2><p>데이터는 보존되며 로그인만 차단된 계정입니다.</p></div><b>{withdrawnAccounts.length}개</b></div><div className="managed-account-list">{withdrawnAccounts.map((account) => <article key={account.id} className="managed-account-card withdrawn"><div className="managed-account-person"><span>{account.full_name.slice(0, 1)}</span><div><h3>{account.full_name}</h3><p>{emailByUserId.get(account.id) ?? '이메일 정보 없음'} · {roleLabel[account.role] ?? account.role}</p><small>{account.withdrawn_at ? `${new Date(account.withdrawn_at).toLocaleDateString('ko-KR')} 탈퇴` : '비활성 계정'}{account.withdrawal_note ? ` · ${account.withdrawal_note}` : ''}</small></div><b className="account-role withdrawn">로그인 차단</b></div><form action={restoreAccount} className="restore-account-form"><input type="hidden" name="user_id" value={account.id} /><button type="submit">계정 복구</button></form></article>)}{withdrawnAccounts.length === 0 && <div className="account-empty">탈퇴 처리된 계정이 없습니다.</div>}</div></section>

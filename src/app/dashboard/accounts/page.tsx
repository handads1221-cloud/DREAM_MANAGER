...
export default async function AccountsPage({ searchParams }: PageProps<'/dashboard/accounts'>) {
  ...
  const [{ data: requests }, { data: accounts }, { data: assignedRoles }] = await Promise.all([
    supabase.from('registration_requests').select('user_id, email, full_name, phone, address, note, requested_role, rejection_reason, status, requested_at').order('requested_at', { ascending: false }),
  ...
  return <DashboardShell profile={{ full_name: profile.full_name, role: 'admin' }} activeHref="/dashboard/accounts">
    ...
    <section className="approval-section"><h2>가입 승인 대기</h2>{pending.length === 0 ? <div className="account-empty">현재 승인 대기 중인 가입 신청이 없습니다.</div> : <div className="approval-list">{pending.map((request) => <article key={request.user_id} className="approval-card">
      ...
      <div className="approval-actions"><form action={approveRegistration}><input type="hidden" name="user_id" value={request.user_id} /><RoleChecks selected={request.requested_role && request.requested_role !== 'unsure' ? [request.requested_role] : ['parent']} /><button type="submit">승인하고 권한 부여</button></form><form action={rejectRegistration} className="rejection-form"><input type="hidden" name="user_id" value={request.user_id} /><label>반려 사유<textarea name="rejection_reason" rows={2} maxLength={300} placeholder="신청자에게 표시할 사유" required /></label><button type="submit" className="reject">사유와 함께 반려</button></form></div>
    </article>)}</div>}</section>
    ...
    <section className="reviewed-section"><h2>가입 처리 내역</h2><div className="reviewed-list">{reviewed.slice(0, 20).map((request) => <div key={request.user_id}><span>{request.full_name} · {request.email}{request.status === 'rejected' && request.rejection_reason ? ` · ${request.rejection_reason}` : ''}</span><b className={request.status}>{request.status === 'approved' ? '승인 완료' : '반려'}</b></div>)}{reviewed.length === 0 && <p>처리된 가입 신청이 없습니다.</p>}</div></section>
  </DashboardShell>;
}

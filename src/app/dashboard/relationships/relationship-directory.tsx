...
import { assignTeacher, linkParent, linkStudentAccount, unassignTeacher, unlinkParent, updateParentProfile, uploadRelationshipPhoto } from './actions';
...
export function RelationshipDirectory({ students, studentAccounts, parents, teachers }: { students: Student[]; studentAccounts: Account[]; parents: Account[]; teachers: Teacher[] }) {
  ...
  const submitPhoto = (formData: FormData) => startPhotoTransition(async () => {
    setPhotoMessage(null);
    let prepared: FormData;
    try { prepared = await compressPhotoInFormData(formData); }
    catch (error) { setPhotoMessage(error instanceof Error ? error.message : '사진을 처리하지 못했습니다.'); return; }
    await uploadRelationshipPhoto(prepared);
  });
  ...
  return <section className="relationship-workspace">
    ...
    {parent && <Modal close={() => setParent(null)} title={`${parent.full_name} 부모님`} eyebrow="PARENT ACCOUNT"><form action={updateParentProfile} className="relationship-modal-form"><input type="hidden" name="parent_id" value={parent.id}/><label><span>아이디(이메일)</span><input value={parent.email ?? ''} readOnly /></label><label><span>이름</span><input name="full_name" defaultValue={parent.full_name} required /></label><label><span>연락처</span><input name="phone" defaultValue={parent.phone ?? ''} /></label><button>부모 정보 저장</button></form><div className="relationship-linked-list"><h3>연결된 자녀</h3>{childrenFor(parent.id).map((child) => <form key={child.id} action={unlinkParent} onSubmit={(event) => { if (!window.confirm('관계를 삭제하시겠습니까?')) event.preventDefault(); }}><input type="hidden" name="student_id" value={child.id}/><input type="hidden" name="parent_id" value={parent.id}/><span>{child.grade}학년 {child.full_name}<button type="submit" aria-label={`${child.full_name} 관계 해제`}>×</button></span></form>)}{childrenFor(parent.id).length === 0 && <p>연결된 자녀가 없습니다.</p>}</div><form action={linkParent} className="relationship-modal-form compact"><input type="hidden" name="parent_id" value={parent.id}/><label><span>자녀 연결 추가</span><select name="student_id" required><option value="">학생 선택</option>{students.map((child) => <option key={child.id} value={child.id}>{child.grade}학년 {child.full_name}</option>)}</select></label><label><span>관계</span><select name="relationship" defaultValue="" required><option value="" disabled>관계 선택</option><option value="부">부</option><option value="모">모</option><option value="조부">조부</option><option value="조모">조모</option></select></label><button>자녀 연결</button></form></Modal>}
    {teacher && <Modal close={() => setTeacher(null)} title={`${teacher.full_name} 담임반 연결`} eyebrow="TEACHER ACCOUNT"><div className="relationship-linked-list"><h3>현재 담당</h3>{teacher.assignments.map((a, index) => <form key={`${a.grade}-${a.class_name}-${index}`} action={unassignTeacher} onSubmit={(event) => { if (!window.confirm('담당 반 배정을 취소하시겠습니까?')) event.preventDefault(); }}><input type="hidden" name="teacher_id" value={teacher.id}/><input type="hidden" name="grade" value={a.grade}/><input type="hidden" name="class_name" value={a.class_name}/><span>{a.grade}학년 {a.class_name}<button type="submit" aria-label={`${a.grade}학년 ${a.class_name} 담당 취소`}>×</button></span></form>)}{teacher.assignments.length === 0 && <p>아직 배정된 담당반이 없습니다.</p>}</div><form action={assignTeacher} className="relationship-modal-form compact"><input type="hidden" name="teacher_id" value={teacher.id}/><label><span>학년</span><select name="grade">{[1,2,3,4,5,6].map((item) => <option key={item} value={item}>{item}학년</option>)}</select></label><label><span>반</span><input name="class_name" defaultValue="전체" /></label><button>담임반 연결</button></form></Modal>}
    {photoTarget && <Modal close={() => !photoPending && setPhotoTarget(null)} title={`${photoTarget.name} 사진 추가`} eyebrow="PROFILE PHOTO">{photoMessage&&<p className="student-form-message error" role="alert">{photoMessage}</p>}<form action={submitPhoto} className="relationship-modal-form"><input type="hidden" name="target_id" value={photoTarget.id}/><input type="hidden" name="target_kind" value={photoTarget.kind}/><label><span>JPG·PNG·WEBP, 큰 사진은 약 4MB로 자동 압축</span><input type="file" name="photo" accept="image/jpeg,image/png,image/webp" required disabled={photoPending}/></label><button disabled={photoPending}>{photoPending?'사진 처리 중…':'사진 저장'}</button></form></Modal>}
  </section>;
}

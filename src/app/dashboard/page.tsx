...
export default async function DashboardPage({ searchParams }: PageProps<'/dashboard'>) {
  ...
  let parentUpcomingPlans: { id: string; schedule_date: string; schedule_time: string | null; title: string }[] | null = null;
  if (role === 'admin' || role === 'teacher') {
    ...
    const [{ count: attendanceCount }, { data: balances }, { count: inquiryCount }, { data: upcomingPlans }] = await Promise.all([childIds.length ? supabase.from('attendance_records').select('*', { count: 'exact', head: true }).in('student_id', childIds).in('status', ['present','late']) : Promise.resolve({ count: 0 }), childIds.length ? supabase.from('student_point_balances').select('balance').in('student_id', childIds) : Promise.resolve({ data: [] }), supabase.from('inquiries').select('*', { count: 'exact', head: true }).eq('parent_id', userId), supabase.from('weekly_plans').select('id,schedule_date,schedule_time,title').gte('schedule_date', today).order('schedule_date').order('schedule_time').limit(5)]);
    parentUpcomingPlans = upcomingPlans ?? [];
    const balance = (balances ?? []).reduce((sum, item) => sum + Number(item.balance), 0);
    stats = [{ label: '연결된 자녀', value: `${childIds.length}명`, href: '/dashboard/children', tone: 'mint' }, { label: '자녀 출석 기록', value: `${attendanceCount ?? 0}회`, href: '/dashboard/children', tone: 'blue' }, { label: '자녀 보석 합계', value: `${balance}개`, href: '/dashboard/children', tone: 'yellow' }, { label: '문의', value: `${inquiryCount ?? 0}건`, href: '/dashboard/messages', tone: 'pink' }];
    ...
    const { data: student } = await supabase.from('students').select('id, grade, photo_path').eq('profile_id', userId).maybeSingle();
    ...
    if (student?.photo_path) {
      const { data: signedPhoto } = await supabase.storage.from('face-photos').createSignedUrl(student.photo_path, 3600);
      studentPhotoUrl = signedPhoto?.signedUrl ?? null;
    }
    studentHighlights = { gems: Number(balance?.balance ?? 0), attendance: attendanceCount ?? 0 };
    stats = [];
  }
